    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter out = response.getWriter();
         ...
         try {
            ...
            String v4 = request.getParameter("mobile");
            String v5 = request.getParameter("addr");
          
            
            
            Connection con = DbConnector.getConnection();
            Statement st = con.createStatement();
            String ins = "insert into adminreg values('"+v1+"','"+v2+"','"+v3+"','"+v4+"','"+v5+"')";
            int i = st.executeUpdate(ins);
            if(i!=0){
                response.sendRedirect("HomePage.jsp?msg=registered successfully...!");
            }
            else{
                response.sendRedirect("adminRegister.jsp?msg=not registered..!!");
                }
